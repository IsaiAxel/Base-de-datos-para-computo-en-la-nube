...
##Insercion de doc más complejos con doc anidados y ID
json
db.alumnos.insertOne(
 {
nombre: 'Valeria',
...
[
  {
    ...
    equipo: 'Moterrey',
    aficiones: [ 'Dinero', 'Hombres', 'Fiestas' ],
    talentos: { futbol: true, 'bañarse': false }
  }
]
...
Dosposibles operaciones de AND
...
1. Seleccionar los documentos, mostrar el título y la editorial
...
2. Eliminar los odcumentos dpnde laa cantidad sea mayor o igual a 150

db.libros.deleteMany({cantidad:{$gte:150}})

# Expresiones regulares 

1. Buscae los libros que contengan la letra t
...
4. Todos los docuemnetos que en el tiulo comiencen e  J
...
db.libros.find({titulo:{$regexe: 'para'}})

db.libros.find({titulo:{$regex:/JSON/}})

-Distinguir entre mayusculas y minisculas

db.libros.find({titulo:{$regex:/JSON/}}) -> No distingue

db.libros.find({titulo:{$regex:/JSON/, $options:'i'}})

-Seleccionartosos los libros que comiencen con J o j

db.libros.find({titulo:{$regex: RegExp('^j', 'i')}})
...
# Metodos sort (Ordenar Documento)

1. Ordenar los libros de manera ascendente por el precio
db.libros.find({},{titulo:1 ,precio:1,_id:0}).sort({precio:1})

2. Ordenar los libros de manera descendente
db.libros.find({},{titulo:1 ,precio:1,_id:0}).sort({precio:-1})

3. Ordenar los libros de manera ascendente por la editorial y de manera descendente por el precio, mostrando el titulo y la editorial

db.libros.find({},{titulo:1 ,precio:1,editoral:1,_id:0}).sort({precio:-1, editoral:1})

# otros Metodos skip,limit,size

db.libros.find({},{titulo1,precio:1, _id:0,editorial}).size()

--Buscar todos los libros pero mostrando los dos primeros

db.libros.find({},{titulo:1 ,precio:1,_id:0}).limit(2)

--Mostrar los 3 ultimos libros 
db.libros.find({},{titulo:1 ,precio:1,_id:0}).sort({precio:-1}).limit(3)

-Seleccionar todos los libros ordenados por titulo de forma descendente saltando los primeros 2 primeros documentos y mostrando el numero de registros 
db.libros.find({},{titulo:1 ,precio:1,_id:0}).sort({titulo:-1}).skip(2).size()
